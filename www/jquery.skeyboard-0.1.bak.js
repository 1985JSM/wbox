	...
	$.fn.sKeyboard = function(options) {
		...
		this.each(function() {
			...
			*/
			...
			var obj_layer = $("#sKeyboard-layer");
			var obj_button = $("button", obj_layer);
			...
			*/
			var init = function() {
				...
				for(var i = 0 ; i < 3 ; i++) {
					...
					if(i == 2) {
						layer_content += "<li><button type=\"button\" class=\"sKeyboard-shift\">Shift</button></li>";
					}
					...
					if(i == 2) {
						layer_content += "<li><button type=\"button\" class=\"sKeyboard-backspace\">Backspace</button></li>";
					}
				...
				layer_content += "<li><button type=\"button\" class=\"sKeyboard-symbol\">기호</button></li>";
				layer_content += "<li><button type=\"button\" class=\"sKeyboard-switch\">한영</button></li>";
				layer_content += "<li><button type=\"button\" class=\"sKeyboard-spacebar\">Space</button></li>";
				layer_content += "<li><button type=\"button\" class=\"sKeyboard-character\">.</button></li>";
				layer_content += "<li><button type=\"button\" class=\"sKeyboard-enter\">Enter</button></li>";
			...
			var openKeyboardLayer = function() {
			...
			var onClickListener = function(obj) {
				...
				if(obj.hasClass("sKeyboard-shift")) {
					...
					if(character_mode == "korean") {
						// korean

						var char_code = obj.text().charCodeAt(0);
						alert(char_code);
						
					}
					else {
						// number & english & symbol
						addLetterWord(obj.text());
					}
					*/
				}
