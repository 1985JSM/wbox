...
$user_std_arr = $oStats->get('user_std_arr');
if(!$std_type) {
	$std_type = 'area';
}

$stats = $oStats->selectUserJoinStats($std_type);
...
<div id="<?=$module?>">
	...
	<!-- search -->
	<div class="search">
		<form name="search_form" action="./user_join.html" method="get" onsubmit="return submitSearchForm(this)">
		...
		<tr>
			<th>지역설정</th>
			<td>
				<select name="sch_area" class="select" title="시/도">
				<option value="">전체 지역</option>
				<? printSelectOption($area_arr, $sch_area, 1); ?>
				</select>				
			</td>
		</tr>
		<tr>
		...
		<tr>
			<th>집계기준</th>
			<td>
				<? printRadio('std_type', $user_std_arr, $std_type, 1); ?>
			</td>
		</tr>
		</tbody>
		</table>
		...
		<p class="button">		
			...
			<a href="./user_join.html" class="sButton" title="초기화">초기화</a>
		</p>
		</form>
	</div>
	<!-- //search -->
	...
	<div class="stats">
		<h4>
			<strong class="info"><?=$user_std_arr[$std_type]?></strong> 
			<strong>회원가입현황</strong> 
