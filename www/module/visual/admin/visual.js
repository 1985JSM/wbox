...
function changeOrder(direction, obj) {
	...
	var vs_id = $(obj).parent("td").find("input[name=vs_id]").val();
	
	$.ajax({
		url : "./process.html",
		type : "post",
		dataType : "json",
		data : "flag_json=1&mode=change_order&direction=" + direction + "&vs_id=" + vs_id,
		cache: false,
		async: false,
...
function chooseRecoShop(sh_code, sh_name) {

	var f = document.write_form;
	f.sh_code.value = sh_code;
	f.sh_name.value = sh_name;

	closeLayerPopup();
}

function deleteRecoShop() {
