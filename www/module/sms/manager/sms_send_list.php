	...
	<div class="subWrap sub_send">		
		...
		<div class="search">
			...
			<tr>
				<th>메시지유형</th>
				<td>
					<select name="sch_ms_type" class="select" title="메시지유형">
						<option value="">메시지유형</option>
						<? printSelectOption($oSms->get('sch_ms_type'), $sch_ms_type, 1); ?>
					</select>

					<select name="sch_ms_state" class="select" title="상태">
						<option value="">상태</option>
						<? printSelectOption($oSms->get('sch_ms_state'), $sch_ms_state, 1); ?>
					</select>

					<select name="sch_ms_rs_type" class="select" title="전송유형">
						<option value="">전송유형</option>
						<? printSelectOption($oSms->get('sch_ms_rs_type'), $sch_ms_rs_type, 1); ?>
					</select>
				</td>
			</tr>
			<tr>
			...
			<tr>
				<th>검색어</th>
				<td>
					<select name="sch_like" class="select" title="수신번호">
						<option value="">검색어</option>
						<? printSelectOption($oSms->get('sch_like'), $sch_like, 1); ?>
					</select>

					<input type="text" name="sch_text" value="<?=$sch_text?>" class="text" size="30" maxlength="30" title="검색어" />
		...
		<?
		if ($list['cnt_current'] > 0) {
			foreach ($list['list'] as $key => $value) { ?>
				<tr>
					<td><?= $value['ms_sd_time'] ?></td>
					<td><?= $value['ms_receiver_no'] ?></td>
					<td><?= $value['ms_receiver_name'] ?></td>
					<td><?= $value['txt_ms_type'] ?></td>
					<td class="content"><p class="message_con"><?= $value['ms_content'] ?></p></td>
					<td><?= $value['txt_ms_rs_type'] ?></td>
					<td><span class="<?= $value['txt_ms_state_class'] ?>"><?= $value['txt_ms_state'] ?></td>
				</tr>
			<? }
		}
		else {
