...
$now_time = time();
$now_date = date('Y-m-d');
$cnt_today_visit = $oShopVisit->countRows($now_date, $member['sh_code']);

// 어제 방문자
$pre_time = $now_time - 24 * 3600;
$pre_date = date('Y-m-d', $pre_time);
$cnt_pre_visit = $oShopVisit->countRows($pre_date, $member['sh_code']);

# 2. 즐겨찾기
include_once(_MODULE_PATH_.'/favorite/favorite.class.php');
$oFavorite = new Favorite();
$oFavorite->init();
$cnt_total_favorite = $oFavorite->countByShopCode($member['sh_code']);
...
// 누적 예약건수
$cnt_total_reserve = $oReserve->countByShopCode($member['sh_code'], 'W,P,E,C,B');
		...
		<div class="main_box">
			...
			<div class="main_article">
				...
				<tr>
					...
					<td><?=$fv_list[$i]['last_favorite_date']?></td>
				</tr>
