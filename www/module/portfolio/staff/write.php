	...
	<div class="portfolio_add_area">
		...
		<li class="photo">
			...
			<li id="main_img" class="photo_upload<? if($main_img['file_id']) { ?> on<? } ?>">
				<div class="photo_button">
					<button class="btn_camera" onclick="choosePortfolioPhoto('<?=$data['pf_id']?>','main_img')" type="button"><i class="xi-image"></i><span>사진추가</span></button>
				</div>

				<div class="photo_thumb" class="img_area">						
					<img src="<?=$main_img['thumb']?>" alt="main photo" />
					<button class="btn_del_img" onclick="deletePortfolioPhoto('<?=$data['pf_id']?>','main_img')" type="button"><span><i class="xi-close"></i></span></button>
				</div>

				<input type="hidden" name="file_id[]" value="<?=$main_img['file_id']?>" />
		...
		<li class="photo">
			...
			<li id="sub_img<?=$i?>" class="photo_upload<? if($sub_img[($i-1)]['file_id']) { ?> on<? } ?>">
				<div class="photo_button">
					<button class="btn_camera" onclick="choosePortfolioPhoto('<?=$data['pf_id']?>','sub_img<?=$i?>')" type="button"><i class="xi-image"></i><span>사진추가</span></button>
				</div>

				<div class="photo_thumb" class="img_area">						
					<img src="<?=$sub_img[$i-1]['thumb']?>" alt="sub photo<?=$i?>" />
					<button class="btn_del_img" onclick="deletePortfolioPhoto('<?=$data['pf_id']?>','sub_img<?=$i?>')" type="button"><span><i class="xi-close"></i></span></button>
				</div>			
