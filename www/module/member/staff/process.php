...
if($mode == 'login') {
	...
	// 이메일 중복 검사
	$result = $oMember->checkMemberEmail($mb_email, $df_email);	
	echo json_encode($result);
	exit;
}
else if($mode == 'check_member_nick') {
	// 닉네임 중복 검사
	$result = $oMember->checkMemberNick($mb_nick, $df_nick);	
	echo json_encode($result);
	exit;
}
...
else if($mode == 'update_setting') {
	// 근무시간 수정
	$result = $oMember->updateSetting();
	movePage($result['url']);
	exit;
}
